...
graph TB
    ...
    subgraph "Azure Functions (.NET 8)"
        F1[CheckDuplicate API<br/>HTTP Trigger]
        F2[SummarizePost API<br/>HTTP Trigger]
        F3[InsertProcessed API<br/>HTTP Trigger]
        F4[GetRecentPosts API<br/>HTTP Trigger]
        F5[GenerateEmailHtml API<br/>HTTP Trigger]
    end
...
graph LR
    ...
    subgraph "애플리케이션 계층"
        FUNC1[CheckDuplicate<br/>.NET 8 Function]
        FUNC2[SummarizePost<br/>.NET 8 Function]
        FUNC3[InsertProcessed<br/>.NET 8 Function]
        FUNC4[GetRecentPosts<br/>.NET 8 Function]
        FUNC5[GenerateEmailHtml<br/>.NET 8 Function]
    end
    ...
    EMAIL -.->|수신| OFFICE
    LOGICAPP -->|중복 확인| FUNC1
    LOGICAPP -->|요약 생성| FUNC2
    LOGICAPP -->|저장| FUNC3
    LOGICAPP -->|조회| FUNC4
    LOGICAPP -->|HTML 생성| FUNC5
    LOGICAPP -->|읽기| RSS
    LOGICAPP -->|발송| OFFICE
    FUNC1 -->|조회| TABLE
    FUNC2 -->|호출| OPENAI
    ...
    style FUNC3 fill:#E6FFE6
    style FUNC4 fill:#E6FFE6
    style FUNC5 fill:#E6FFE6
...
- **처리 과정**:
  ...
  2. Base64 URL-safe 인코딩 적용 (RowKey 생성)
  3. Table Storage에서 PartitionKey=SourceName, RowKey=해시로 조회
  4. 존재 여부 확인 (404는 신규, 200은 중복)
- **출력**: 
  ```json
  {
    "isDuplicate": false
  ...
  ```
- **목적**: Logic App에서 신규 게시글 여부 판단

#### SummarizePost Function
- **트리거**: HTTP POST
- **엔드포인트**: `/api/SummarizePost`
- **입력**: 블로그 게시글 URL, 제목, 설명
- **처리 과정**:
  1. URL에서 게시글 콘텐츠 추출
  2. Azure OpenAI GPT-4o를 통한 요약 생성
  3. 영문 및 한글 요약 생성 (각 2-3문장)
  4. 핵심 인사이트 추출
- **출력**: JSON 형식의 요약 데이터 (영문/한글 배열)

#### InsertProcessed Function
- **트리거**: HTTP POST
- **엔드포인트**: `/api/InsertProcessed`
- **입력**: 
  ```json
  {
    "title": "게시글 제목",
    ...
    "koreanSummary": ["한글 요약1", "한글 요약2"],
    "englishSummary": ["English summary1", "English summary2"]
  }
  ```
- **처리 과정**:
  1. 게시글 링크를 SHA256 해시로 변환 (RowKey 생성)
  2. PartitionKey=SourceName으로 Table Entity 생성
  3. KoreanSummary, EnglishSummary를 JSON 문자열로 직렬화
  4. ProcessedDate 타임스탬프 추가
  5. UpsertEntity (Replace 모드) - 중복 시 덮어쓰기
- **출력**: 
  ```json
  {
    "success": true,
  ...
  ```
- **목적**: 처리된 게시글을 Table Storage에 저장하여 중복 방지

#### GetRecentPosts Function
- **트리거**: HTTP POST
- **엔드포인트**: `/api/GetRecentPosts?daysSince=1`
- **입력**: 
  - Query String: `daysSince` (기본값: 1)
  - Body: `{ "Limit": 15 }` (기본값: 15)
- **처리 과정**:
  1. 날짜 필터 계산 (현재 시간 - daysSince)
  2. Table Storage 쿼리: `ProcessedDate >= cutoffDate`
  3. 결과 제한 (Limit 값 적용)
  4. KoreanSummary, EnglishSummary JSON 역직렬화
  5. 소스별 이모지 매핑 (11+ 피드 타입 지원)
- **출력**: 게시글 배열 (제목, 링크, 소스, 요약, 이모지 포함)
- **목적**: 신규 게시글이 없을 때 최근 처리된 게시글 조회

#### GenerateEmailHtml Function
- **트리거**: HTTP POST
- **엔드포인트**: `/api/GenerateEmailHtml`
- **입력**: 요약된 게시글 배열
- **처리 과정**:
  1. 신규 게시글 배열 검증 (비어있으면 GetRecentPosts 호출)
  2. 반응형 HTML 이메일 템플릿 생성
  3. 소스별 이모지와 함께 게시글 렌더링
  4. 이메일 제목 생성 ("새 게시글 N개" 또는 "최근 게시글 요약 (신규 없음)")
...
- **Title**: 게시글 제목
- **Link**: 게시글 원본 URL
- **SourceName**: 피드 소스 이름 (PartitionKey와 동일)
- **Summary**: 원본 게시글 설명/요약
- **PublishDate**: 게시 타임스탬프 (ISO 8601 형식)
- **ProcessedDate**: 처리 시간 (ISO 8601 형식)
- **KoreanSummary**: AI 생성 한글 요약 (JSON 문자열 배열)
- **EnglishSummary**: AI 생성 영문 요약 (JSON 문자열 배열)

**목적**: 
- 블로그 게시글 중복 제거 (CheckDuplicate Function에서 조회)
- 처리 이력 추적 (InsertProcessed Function에서 저장)
- 최근 게시글 조회 지원 (GetRecentPosts Function에서 날짜 필터링)
- SHA256 해시 기반 고유성 보장 (URL 변경에도 안정적)

**인덱싱**:
- PartitionKey로 소스별 분리 (효율적인 쿼리)
