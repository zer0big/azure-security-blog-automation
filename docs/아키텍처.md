...
시스템은 **2개의 독립적인 Logic App (Consumption) 워크플로우**로 구성되어 있습니다:

1. **Security Workflow** - 5개 보안 피드 (07:00, 19:00 KST)
2. **Azure/Cloud Workflow** - 7개 Azure 피드 (08:00, 20:00 KST)
...
graph TB
    subgraph "트리거"
        T1["반복 트리거 #1
        Security: 07:00, 19:00 KST"]
        T2["반복 트리거 #2
        Azure/Cloud: 08:00, 20:00 KST"]
    end
    ...
    subgraph "Logic App #2: Azure/Cloud"
        ...
        subgraph "Azure RSS 피드 (7개)"
            RSS6[☁️ Azure Blog]
            RSS7[🔧 Azure DevOps]
            RSS8[📊 Fabric]
            RSS9[🏗️ Azure Infrastructure]
            RSS10[🔨 M365 Dev]
    ...
    RSS6 & RSS7 & RSS8 & RSS9 & RSS10 & RSS11 & RSS12 --> PARSE
    PARSE --> FILTER
    FILTER --> CHECK
    CHECK -->|중복 확인| F1
    F1 -->|신규 게시글| SUM
...
graph LR
    ...
    subgraph "오케스트레이션 계층"
        LOGICAPP["Logic App Consumption\n워크플로우 오케스트레이션"]
    end
...
sequenceDiagram
    ...
    Note over Timer,LA2: Security Workflow (07:00, 19:00)
    ...
    Note over Timer,LA2: Azure Workflow (08:00, 20:00)
    Timer->>LA2: 트리거 (Azure)
    
    loop 각 RSS 피드 (7개 - Azure)
        LA2->>RSS: RSS 피드 가져오기
...
- **반복 트리거**: 매일 08:00, 20:00 KST에 실행
- **RSS 피드 처리**: 7개의 Azure/Cloud 블로그 RSS 피드 수집 및 파싱
  - ☁️ Azure Blog
  - 🔧 Azure DevOps Blog
  - 📊 Fabric Blog
...
- **입력**: 
  ...
  ```
- **처리 과정**:
  1. **RSS 피드 수집**: 모든 피드를 병렬 처리 (Task.WhenAll, FAIL 피드 자동 skip)
...
현재 **12개 피드** (Security 5개 + Azure Cloud 7개)

### Security Workflow (5개 피드)

| 아이콘 | 피드 이름 | URL | 콘텐츠 주제 |
|--------|-----------|-----|------------|
| 🛡️ | Microsoft Security Blog | https://www.microsoft.com/en-us/security/blog/feed/ | 일반 보안 주제 |
| 🔐 | Microsoft Sentinel Blog | https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=MicrosoftSentinelBlog | Sentinel SIEM 플랫폼 |
| 🛡️ | Microsoft Defender Blog | https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=MicrosoftDefenderBlog | Microsoft Defender 업데이트 |
| 🌐 | Zero Trust Blog | https://www.microsoft.com/en-us/security/blog/topic/zero-trust/feed/ | Zero Trust 아키텍처, 조건부 액세스 |
| 🔑 | Identity Blog | https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=Identity | ID 보안, Entra ID |
...
| 🔧 | Azure DevOps Blog | https://devblogs.microsoft.com/devops/feed/ | DevOps, CI/CD, GitHub Actions |
| 📊 | Fabric Blog | https://blog.fabric.microsoft.com/en-us/blog/feed/ | Fabric, 데이터 통합, Analytics |
| 🏗️ | Azure Infrastructure Blog | https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=AzureInfrastructureBlog | 네트워킹, 컴퓨팅, 스토리지 인프라 |
| 🔨 | Microsoft 365 Dev Blog | https://devblogs.microsoft.com/microsoft365dev/feed/ | M365 개발, Graph API |
| ⚡ | Power Platform Blog | https://cloudblogs.microsoft.com/powerplatform/feed/ | Power Apps, Power Automate |
| 🤖 | Azure AI Foundry Blog | https://techcommunity.microsoft.com/t5/s/gxcuf89792/rss/board?board.id=azure-ai-foundry-blog | Azure AI, 모델 배포, AI 서비스 |
...
- **Consumption Plan Logic App**: 실행 기반 과금 (예측 가능한 비용)
...
### 현재 용량
- **RSS 피드**: 12개 피드 (Security 5 + Azure Cloud 7)
- **이메일 빈도**: 하루 4회 (Security 07:00/19:00, Azure Cloud 08:00/20:00 KST)
- **Function 타임아웃**: 10분 (host.json functionTimeout: "00:10:00")
