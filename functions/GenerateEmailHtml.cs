...
public class GenerateEmailHtml
{
        ...
        [HttpTrigger(AuthorizationLevel.Function, "post")] HttpRequest req)
    {
        ...
        try
        {
            ...
            var actualNewPostsCount = displayPosts.Count(p => 
                p.Title != "No new posts in last 24 hours" && 
                p.Title != "No new posts in last 7 days" && 
                p.Title != "No new posts in last 30 days" && 
                p.Summary != "No new posts");
    ...
    private async Task<string> GenerateHtmlAsync(BlogPost[] posts, bool hasNewPosts, int newPostsCount, int recentPostsCount, string scheduleText)
    {
        ...
        if (newPostsCount > 0)
        {
            sb.AppendLine($"<div class=\"count\">신규 {newPostsCount}개</div>");
        }
        else
        {
            sb.AppendLine($"<div class=\"count\">신규 없음 (최근 24시간)</div>");
        }
        ...
        // 실제 오류: posts가 하나도 없는 경우
        if (posts.Length == 0)
        {
            sb.AppendLine("<div class=\"post\">");
            sb.AppendLine("<div class=\"post-title\">⚠️ RSS 피드 조회 오류</div>");
            sb.AppendLine("<div class=\"post-summary\">RSS 피드에서 데이터를 가져오지 못했습니다. Logic App 실행 기록을 확인해 주세요.</div>");
            sb.AppendLine("</div>");
        }
        // 신규 게시글이 없을 때 안내 메시지 (정상 상황)
        else if (!hasNewPosts && newPostsCount == 0)
        {
            sb.AppendLine("<div class=\"no-new-notice\">");
            sb.AppendLine("📌 최근 24시간 내 신규 게시글이 없어 최근 글을 요약해서 보냅니다. (라이브 RSS 기준)");
            sb.AppendLine("</div>");
        }
        ...
        foreach (var post in posts)
        {
            ...
            if (post.Title == "No new posts in last 7 days" || post.Title == "No new posts in last 24 hours" || post.Title == "No new posts in last 30 days" || post.Summary == "No new posts")
            {
                ...
                var sourceName = !string.IsNullOrEmpty(post.SourceName) 
                    ...
                    : "Unknown Source";
                sb.AppendLine($"<div style=\"padding: 10px 20px; margin: 10px 0; color: #666; font-size: 14px; border-left: 3px solid #ddd;\">{emoji} {sourceName}: 신규 없음 (최근 24시간)</div>");
                continue;
            }
...
{
    // Convert emoji to HTML entities for better email client compatibility
    private static string EmojiToHtml(string emoji)
    {
